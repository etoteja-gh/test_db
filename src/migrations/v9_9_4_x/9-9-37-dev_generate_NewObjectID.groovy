...
databaseChangeLog{
	clid = "9-9-37-dev_generate_NewObjectId"

	changeSet(author: "Ken", id: "${clid}.1", dbms:'mssql') {
		createProcedure("""
Create or ALTER procedure dev_generate_NewObjectID as
/**
...
* like outBatchId or CrossRefNum (make sure that's numeric only and then put it in)
**/
declare @globalID int
declare @maxCrossRefNum int
declare @maxOutBatchId int
...
set @maxOutBatchId=(select isnull(max(outBatchId),0) from ArTran)
set @globalID=@maxOutBatchId
set @globalID=
CASE
...
Delete from NewObjectID where KeyName='Customer.num';
